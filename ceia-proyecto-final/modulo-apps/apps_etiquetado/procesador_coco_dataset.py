...
def _convert_image_annotations_to_cvat_annotations(
...
def _convert_patch_annotations_to_cvat_annotations(
    images: Dict[str, Any], annotations: Dict[str, Any]
) -> Tuple[Dict[str, Any], Dict[str, Any]]:
...
def save_coco_annotations(
    ...
    field_name: str,
    annotation_type: Literal["images", "patches", "cvat"] = "cvat",
) -> bool:
    ...
    if annotation_type == "images":
        images, annotations = _convert_image_annotations_to_cvat_annotations(images, annotations)
    elif annotation_type == "patches":
        images, annotations = _convert_patch_annotations_to_cvat_annotations(images, annotations)
...
def load_coco_annotations_from_mongodb(
    ...
    try:
        file_path = (
            self.download_annotations_as_coco_from_mongodb(field_name=field_name, patches_names=patches_names)
            if patches_names
            else self.download_annotations_as_coco_from_mongodb(field_name=field_name, images_names=images_names)
        )
    except Exception as e:
        raise Exception(f"Error al descargar las anotaciones: {e}")
