...
def download_annotations_from_cvat(
    ...
    try:
        ...
        try:
            ...
            if output_filename is None:
                output_filename = DOWNLOAD_TEMP_FOLDER / f"cvat_task_{retrieved_id}.zip"
    ...
    if job_id:
        DOWNLOAD_JOBS_FOLDER.mkdir(parents=True, exist_ok=True)
        output_json_file = DOWNLOAD_JOBS_FOLDER / f"cvat_job_{job_id}.json"
    else:
        DOWNLOAD_TASKS_FOLDER.mkdir(parents=True, exist_ok=True)
        output_json_file = DOWNLOAD_TASKS_FOLDER / f"cvat_task_{task_id}.json"
...
def convert_image_annotations_to_cvat_annotations(
    ...
    Recibe una lista de imágenes y sus anotaciones, y las convierte a sus parches. Esto posibilita subir las anotaciones a CVAT.

    Se utiliza para cargar directamente las anotaciones en que las imágenes es la imagen completa y no parches.
